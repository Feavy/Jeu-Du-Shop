...
public class CraftManager {
    private final static Map<ItemType, Craft> CRAFTS = new EnumMap<ItemType, Craft>(ItemType.class);

    private static void registerCraft(@NotNull Craft craft) {
        CRAFTS.put(craft.getResultItem().getType(), craft);
    }

    public static Craft[] getRealisableCrafts(@NotNull Inventory inventory) {
        return (Craft[]) CRAFTS.values().stream().filter(craft -> craft.isRealizable(inventory)).toArray();
    }

    public static boolean canCraft(@NotNull ItemType itemType, @NotNull Inventory inventory) {
     ...
     * @return
     */
    public static Optional<Craft> getRealizableCraft(@NotNull ItemType itemType, @NotNull Inventory inventory) {
